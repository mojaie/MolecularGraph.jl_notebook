...
using Pkg
Pkg.add(PackageSpec(name="MolecularGraph", version="0.8"))
...
for (name, query) in queries
    ...
    suite["exact"] = @benchmarkable [isstructmatch(m, $query) for m in $mols]
    suite["substruct"] = @benchmarkable [issubstructmatch(m, $query) for m in $mols]
    tune!(suite)
...
display(data)
outpath = joinpath(ENV["OUTPUT_DIR"], "bstructsearch_08.csv")
